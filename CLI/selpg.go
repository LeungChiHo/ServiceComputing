...
type selpg_args struct {
	...
	in_filename string 
	page_len    int    //默认5
	page_type   bool //有-f为true，默认为false 
	print_dest  string
...
func Parser(p *selpg_args) {
	flag.Usage = usage
	flag.IntVar(&p.start_page, "s", -1, "首页")
	flag.IntVar(&p.end_page, "e", -1, "尾页")
	flag.IntVar(&p.page_len, "l", 5, "打印的每页行数")
	flag.BoolVar(&p.page_type, "f", false, "是否用换页符换页")
	flag.StringVar(&p.print_dest, "d", "", "打印目的地")
	flag.Parse()
}
...
func usage() {
	...
	fmt.Fprintf(os.Stderr,"\tselpg -sNumber -eNumber [options] [filename]\n\n")
